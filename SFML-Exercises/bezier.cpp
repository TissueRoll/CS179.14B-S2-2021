...
bool readFromAvailableText() {
    std::string input;
    std::ifstream settings("bezier.txt");
    if (settings.is_open()) {
    	settings >> smoothness;
...
void initializeSettings() {
    if (readFromAvailableText()) {
        std::cout << "bezier.txt successfully loaded.\n";
    } else {
        std::cout << "bezier.txt not loaded. Using default values.\n";
        circles.resize(control_points);
    	for (unsigned int i = 0; i < control_points; ++i) {
    		circles[i].setRadius(c_radius);
    		circles[i].setOrigin(c_radius, c_radius);
...
int main () {
    srand(time(NULL));
    sf::RenderWindow window(sf::VideoMode(window_w, window_h), "Bezier-01");
